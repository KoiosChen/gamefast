...
def check_pcap_result():
    ...
    elif json.loads(redis_record)['status'] == 1:
        return jsonify({"status": 'fail', "content": "正在处理中，请稍后"})
    elif json.loads(redis_record)['status'] == 2:
        return jsonify({"status": 'ok', "content": "处理完成，查看结果"})
    elif json.loads(redis_record)['status'] == 3:
...
def do_upload_pcap():
    ...
    if request.method == 'GET':
        ...
        for f in files:
            ...
            if try_this_order:
                print(try_this_order)
            file_saved = uploadfile(name=f,
                                    size=size,
                                    service_url='_pcap',
                                    order_status=json.loads(try_this_order)[
